...
export default class SchedulerDayConstraints {
  ...
  */
  ...
  isEventFootprintAllowed(
    ...
    for(let i = 0; i < overlapEventFootprints.length; i ++) {
      let overlapFootprint = overlapEventFootprints[i]
      let componentResourceId = componentFootprint.resource.id
      let overlapEventDef = overlapFootprint.eventDef
      let overlapEventResourceId = overlapEventDef.miscProps.resourceId
      let overlapEventType = overlapEventDef.e

      if(componentResourceId == overlapEventResourceId) {
        if(eventFootprint.eventDef.e == 'bo') {
          return false
        } else if(overlapEventType == 'bo') {
          return false
        } else if(overlapEventType == eventFootprint.eventDef.e) {
          return false
        }
      }
    }
  ...
  isFootprintAllowed(
    ...
    for(let i = 0; i < overlapEventFootprints.length; i ++) {
      let eventFootprint = overlapEventFootprints[i]
      let componentResourceId = componentFootprint.resource.id
      let eventDef = eventFootprint.eventDef
      let eventResourceId = eventDef.miscProps.resourceId
      let eventType = eventDef.e

      console.log(eventType, eventResourceId, componentResourceId)
      if(componentResourceId == eventResourceId) {
        if(eventType == 'bo') {
          return false;
        } else if(eventType == 'ap') {
          return false;
        }
      }
    }
